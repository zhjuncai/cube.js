...
export async function proxyCommand(program: CommanderStatic, command: string) {
  ...
  const serverPackageExists = packageExists('cubejs-server', relative);

  const commandInfo = program
    .command(command);

  if (serverPackageExists) {
    const packageManifest = await requirePackageManifest<any>('cubejs-server', {
      relative,
    });
    if (packageManifest.cubejsCliVersion) {
      ...
      if (semver.satisfies(cliManifest.version, packageManifest.cubejsCliVersion)) {
        const OriginalCommandPackage = requireFromPackage<any>(
          `cubejs-server/dist/command/${command}.js`,
          {
            relative,
          }
        );
      ...
      const message = `${chalk.red('Unavailable.')} cubejs-server inside current directory requires ` +
    ...
    const message = `${chalk.red('Unavailable.')} Please upgrade cubejs-server.`;
